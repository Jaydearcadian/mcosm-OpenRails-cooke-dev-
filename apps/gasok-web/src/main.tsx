...
function RouteReset() {
  ...
  useLayoutEffect(() => {
    ...
    const reset = () => {
      ...
      document.querySelectorAll<HTMLElement>('[data-route-scroll]').forEach((element) => { element.scrollTop = 0; });
    };
    reset();
    let secondFrame = 0;
  ...
  return (
    <div className={`app-shell ${docsMode ? 'docs-mode' : ''}`}>
      ...
      <header className={`control-strip ${compact ? 'is-compact' : ''}`}>
        ...
        <div className="network-control"><span><i /> {ACTIVE_NETWORK.displayLabel}</span><button type="button" onClick={() => void connect()}>{connecting ? 'CONNECTING' : address ? `${address.slice(0, 6)}…${address.slice(-4)}` : 'CONNECT'}</button></div>
      </header>
      {children}
      {!docsMode && <Footer />}
    </div>
...
function AppRoutes() {
  ...
  return (
    <div id="route-content" className="route-surface" tabIndex={-1}>
