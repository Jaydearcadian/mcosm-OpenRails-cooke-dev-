...
contract ArcOpenRailsHubV1 {
    ...
    struct PaycardRegistry {
        address payer;
        address recipient;
        bytes32 metadataHash;          // Canonical invoice/card terms hash
    ...
    constructor(address _arcUsdcNativeGasAddress) {
        if (_arcUsdcNativeGasAddress == address(0)) revert InvalidIntent();
        arcUsdc = IERC20_ArcGas(_arcUsdcNativeGasAddress);

        // Ownable2Step: set deployer as initial owner
        owner = msg.sender;
        emit OwnershipTransferred(address(0), msg.sender);

        // Reentrancy Guard: initialise lock state
        _status = _NOT_ENTERED;
    ...
    /**
     * @notice Opens a RailsCard wildcard intent by binding the claimant as the recipient.
     * @dev The payer must have signed the intent with `recipient == address(0)`.
     */
